...
async def chat(request: ChatRequest):
    ...
    if any(q in user_msg for q in ["who made you", "ಯಾರು ಕ್ರಿಯೇಟ್ ಮಾಡಿದ್ದು", "ಯಾರು ಮಾಡಿದ್ದು", "madiddu"]):
        return {"reply": "Nagesh Nirale made me."}
    if any(q in user_msg for q in ["what languages", "ಯಾವ ಭಾಷೆ", "language"]):
        return {"reply": "I can understand and communicate in many languages including Kannada, English, Hindi, and more."}

    contents = [request.message if request.message else "Explain this image"]
    if request.image_data:
        contents.append(types.Part.from_bytes(data=base64.b64decode(request.image_data), mime_type="image/jpeg"))
...
.listening { color: #ff4444 !important; animation: pulse 1.5s infinite; }
@keyframes pulse { 0% { opacity: 1; } 50% { opacity: 0.4; } 100% { opacity: 1; } }
.send { background: #a8c7fa; color: #131314; border: none; width: 38px; height: 38px; border-radius: 50%; cursor: pointer; font-weight: bold; display: flex; align-items: center; justify-content: center; font-size: 16px; }
